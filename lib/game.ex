defmodule ExMon.Game do
  ...
  def update(state) do #recebe um estado e atualiza pelo novo
    Agent.update(__MODULE__, fn _ -> update_game_status(state)  end)
  end

  def player, do: Map.get(info(), :player)   #Retorna qual é o jogador da vez
  ...
  defp update_turn(%{turn: :player} = state), do: Map.put(state, :turn, :computer) #troca o turno entre os jogadores
